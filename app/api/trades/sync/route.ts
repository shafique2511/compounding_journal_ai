...
const tradeSyncSchema = z.object({
  trades: z.array(z.custom<Trade>((value) => isTradePayload(value))).default([]),
});
...
export async function POST(request: Request) {
  try {
    ...
    if (userError || !user) {
      return NextResponse.json({ error: "Login is required to save trades." }, { status: 401 });
    }

    const serviceClient = requireSupabaseServiceRoleClient();
    ...
    const { data: conflictingRows, error: conflictError } = await serviceClient
      .from("trades")
      .select("id,user_id")
      .in("id", tradeIds)
      .neq("user_id", user.id);
    ...
    if (conflictError) {
      logTechnicalError(conflictError, { action: "check trade ownership", source: "database" });
      return NextResponse.json({ error: "Could not verify trade ownership." }, { status: 500 });
    }

    if ((conflictingRows ?? []).length > 0) {
      return NextResponse.json({ error: "You do not have permission to update one or more trades." }, { status: 403 });
    }

    const { data, error } = await serviceClient
      .from("trades")
      .upsert(trades.map((trade) => tradeToInsert(trade, user.id)), { onConflict: "id" })
      .select("*")
      .eq("user_id", user.id)
    ...
    if (error) {
      ...
      return NextResponse.json({ error: "Trades could not be saved to Supabase." }, { status: 500 });
    }
